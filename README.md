...
- 嵌套路由（Nested Routing）是一种在Web应用中组织和实现页面路由的方法，它允许开发者创建一个层次化的页面结构，其中某个路由下的子页面可以继承并扩展父路由的布局或功能。这种方式非常适合开发具有多层次页面结构的应用，比如具有多个子页面或子部分的仪表板、带有多层分类的博客等
...
- 当你使用这个装饰器在一个类的属性上时，这个属性的值就必须是大于0的数字。如果属性的值不满足这个条件，那么 class-validator 将会产生一个验证错误，这可以帮助开发者确保数据的有效性和符合预期的类
